...
parser = argparse.ArgumentParser(description='Training the distribution network for MNIST')
parser.add_argument('--seed', default=1, type=int, metavar='N',  help='the random seed')
parser.add_argument('-b', '--batch-size', default=128, type=int, metavar='N', help='mini-batch size (default: 128)')
parser.add_argument('--path', default='/home/hddraid/shared_data/MNIST/', type=str, help='data path')
parser.add_argument('--savepath', default='/home/hddraid/opensetmodel/MNIST/mnist_map_tr7/', type=str, help='path to save models')
...
val_label=range(8)
test_label=range(10)
train_set = MinstDataset(path=args.path, use='train', label=train_label)
val_set = MinstDataset(path=args.path, use='validation', label=val_label)
test_set = MinstDataset(path=args.path, use='test', label=test_label)
...
model = MyVggNet16_bn_MNIST(outnum=ndim).cuda()
